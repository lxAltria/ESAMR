...
namespace MDR {
    ...
    class ZSTD : public concepts::LosslessCompressorInterface {
    public:
        ZSTD(){}
        uint32_t compress(uint8_t* data, uint32_t dataLength, uint8_t** compressBytes) const {
            ...
            *reinterpret_cast<size_t*>(*compressBytes) = dataLength;
            outSize = ZSTD_compress(*compressBytes + sizeof(size_t), estimatedCompressedSize, data, dataLength, level); 
        ...
        void print() const {
            std::cout << "ZSTD lossless compressor" << std::endl;
        }
    private:
        const int level = 3; //default setting of level is 3
    };
}
#endif
