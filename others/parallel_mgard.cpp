...
int main(int argc, char ** argv){
    ...
    std::vector<size_t> dims{(uint32_t)n1, (uint32_t)n2, (uint32_t)n3};
    const int target_level = 5;
    for (int i=0; i<num_reb; i++) {
        adios2::ADIOS ad(MPI_COMM_WORLD);
        adios2::IO bpIO = ad.DeclareIO("WriteMGARD");
        std::string zip_filename = "p" + std::to_string(size) + "_mgard/eb_1e-" + std::to_string(i+1);
        // Compress Input Data
        if (rank == 0){
        ...
        MGARD::Decomposer<float> decomposer(false);
        unsigned char * compressed_data = decomposer.compress(data, dims, target_level, abs_bound[i], compressed_size[i]);
        MPI_Barrier(MPI_COMM_WORLD);
        if(rank == 0){
            compress_time[i] += MPI_Wtime();
        }
    ...
    for(int i=0; i<num_reb; i++){
        adios2::ADIOS ad(MPI_COMM_WORLD);
        adios2::IO readIO = ad.DeclareIO("ReadMGARD");
        std::string zip_filename = "p" + std::to_string(size) + "_mgard/eb_1e-" + std::to_string(i+1);
        ...
        float * decompressed_data = recomposer.decompress(compressed_data, compressed_size[i], dims);
        MPI_Barrier(MPI_COMM_WORLD);
        if(rank == 0){
            decompress_time[i] += MPI_Wtime();
        }
    ...
    if (rank == 0){
        printf ("MGARD Finish parallel compressing\n");
